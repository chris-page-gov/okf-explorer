...
verified; selective CPSV-AP 3.2.0 adoption authorised; the next producer review
is `okf-LandRegistry`, 10 August 2026.
...
| Repository or contract | State on 10 August 2026 | Next evidence required |
| --- | --- | --- |
| `okf-explorer` shared authoring contract | Implemented locally in this guide with regression coverage; publication of these changed documentation bytes remains pending. | Commit, Pages deployment and exact public-byte verification. |
| `okf-LandRegistry` | Implemented locally for eight evidence-selected public services, with eight `cv:hasCompetentAuthority` assertions. The exact CPSV-AP 3.2.0 context, vocabulary and SHACL file are vendored and locked; the official SHACL shapes are explicitly recorded as `not-run`. | Independent semantic review, full G1–G8 candidate evidence, exact-digest G9 approval, deployment and real-Reader verification. |
...
| --- | --- | --- | --- |
| `okf-explorer` | Contract and v0.6.0 release complete; canonical vendoring hardening merged through PR #80 as `50078164`; this later CPSV-AP policy change is local | 155 entities and 579 conservative `dcterms:references` assertions in synchronised YAML-LD, JSON-LD and compatibility runtime projections; the merged follow-up pins the canonical profile's complete 16-file byte identity without changing those profile bytes | Review, commit and publish this documentation-only policy update independently |
...
| `okf-LandRegistry` | Full metadata-level implementation completed locally; unreleased candidate | 2,203 records and 2,212 exact-schema-valid direct/reified/runtime assertions: 2,203 publisher mappings, eight CPSV-AP 3.2.0 competent-authority mappings and one Welsh-to-English `schema:translationOfWork` mapping. The active rich runtime uses nine digest-bound chunks and a SHA-256 route locator; YAML-LD and JSON-LD are equivalent. | Complete independent review and fresh G1–G8 evidence, then request exact-digest G9 approval before deployment |
