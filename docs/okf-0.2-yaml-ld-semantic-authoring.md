...
with the first producer's byte-exact profile alignment complete and its
verification and final regeneration in progress, 10 August 2026.
...
| `okf-ai-infrastructure` | Review continues against Explorer v0.6.0; the large v0.6.0 migration remains local and uncommitted. The retrospective annotated [v0.5.0 release](https://github.com/chris-page-gov/okf-ai-infrastructure/releases/tag/v0.5.0) now records the exact historical OKF 0.2 milestone. The producer's 16-file Bundle Wiki v1 mirror and adjacent vendor lock have been synchronised byte for byte with the frozen canonical profile by the merged PR #80 tooling, and its British English source gate passes. Verifier hardening and final regeneration remain in progress; no v0.6.0 commit or release is claimed. | Complete fail-closed profile-lock and schema-pin verification, set one truthful final build instant, regenerate every projection and receipt, and run the exhaustive local and cross-repository gates before opening the independent v0.6.0 producer PR. |
...
| `okf-ai-infrastructure` | Semantic migration and byte-exact canonical profile/lock sync present locally; retrospective annotated v0.5.0 release complete; v0.6.0 verifier hardening remains in progress and uncommitted | The authored migration targets 155 entities, including 142 production concepts, and 579 direct/reified/runtime relationships. The profile mirror and lock now match the frozen canonical bytes, but the final schema pin, receipt and regenerated projections are still being completed, so this is not yet a v0.6.0 release candidate | Finish verifier hardening, assign one truthful build instant, regenerate every projection and receipt, run exhaustive validation, then use the independent PR, release and public-verification gates |
