...
Status: Explorer v0.6.0 released; first producer review in progress, 10 August 2026.
...
| `okf-ai-infrastructure` | Review started against Explorer v0.6.0; the large local migration remains uncommitted. Its vendored Bundle Wiki v1 profile is self-consistent but not identical to the frozen canonical profile: the relationship assertion and concept schemas, shapes and profile guide have drifted while retaining the same profile URI. | Choose and enforce the vendoring policy, align exact canonical files or assign a distinct profile URI, refresh the pinned schema digest, complete the British English source review, regenerate every projection and validate before any commit or release. |
