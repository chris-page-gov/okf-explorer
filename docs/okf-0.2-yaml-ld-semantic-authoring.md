...
Status: Explorer v0.6.0 released; canonical profile vendoring hardening is
implemented locally and first-producer alignment is pending, 10 August 2026.
...
| Canonical Bundle Wiki v1 vendoring | Policy chosen and implemented locally in an unreleased Explorer follow-up: the 16 regular files in `profiles/bundle-wiki/v1/` are an opaque, byte-exact copy of the v0.6.0 tree `d26ae9a818041ff74c469e653ec714632ddbfc2a`. The adjacent `profiles/bundle-wiki/v1.vendor-lock.json` records the canonical URI, release commit, tree, each sorted file size and SHA-256, and one aggregate identity. Reconciliation fails closed on a missing, extra or drifted mirror, checks every declared relationship-schema output, and provides explicit symlink-safe sync with opt-in replacement. A schema retaining the canonical `$id` must retain the canonical bytes; an intentional extension needs its own absolute `$id`, and multiple outputs may share an `$id` only when their bytes are identical. The 27-case focused adversarial suite, the 86-test related Python suite and a strict local reconciliation audit pass. | Complete review and CI, then merge this tooling independently before modifying the first producer. No merge is claimed yet. |
...
| `okf-ai-infrastructure` | Review started against Explorer v0.6.0; the large local migration remains uncommitted. Its local Bundle Wiki v1 mirror is self-consistent but not byte-identical to the frozen canonical profile while retaining the same URI. The full-mirror vendoring policy is now chosen, but the producer has not yet been aligned. | After the Explorer follow-up is reviewed and merged, run the explicit profile sync with replacement, refresh the schema digest and validation receipt, complete the British English source review, regenerate every projection and validate before any commit or release. |
...
| --- | --- | --- | --- |
| `okf-explorer` | Contract and v0.6.0 release complete; follow-up vendoring hardening implemented locally but not merged | 155 entities and 579 conservative `dcterms:references` assertions in synchronised YAML-LD, JSON-LD and compatibility runtime projections; the follow-up pins the canonical profile's complete 16-file byte identity without changing those profile bytes | Review, CI and merge the independent vendoring-tool follow-up |
| `okf-ai-infrastructure` | Semantic migration present locally; canonical profile alignment pending | 155 entities, including 142 production concepts, and 579 direct/reified/runtime relationships were generated, but its same-URI profile mirror and pinned receipt still differ from the frozen v0.6.0 canonical bytes | Apply the reviewed full-mirror sync, refresh the receipt and generated projections, then rerun exhaustive validation and existing preview/release gates |
