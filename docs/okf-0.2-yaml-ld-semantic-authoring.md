...
| Explore OKF authoring method | Reviewed, implemented and merged for Explorer v0.7.0. [PR #96](https://github.com/chris-page-gov/okf-explorer/pull/96) applies the Land Registry retrospective without reopening that pack and adds Authoring Profile v1, the strict [Explore OKF profile](../profiles/explore-okf/v1/index.md), tooling, prompts and beginner guidance. [PR #98](https://github.com/chris-page-gov/okf-explorer/pull/98) preserves `.nojekyll` so the Pages archive matches its complete candidate receipt. Final public verification then exposed a slow-start URL-state race; [PR #99](https://github.com/chris-page-gov/okf-explorer/pull/99) separates the editable URL draft from loaded-source identity and adds deterministic regressions. No v0.7.0 tag or release is claimed while that last correction remains under exact-head review. The [open-data shortlist](../research/explore-okf-open-data-test-candidates.md) recommends a bounded Coventry everyday-services pilot. | Complete PR #99, rebuild and verify the exact-merge Pages archive and public Chrome journeys, then publish immutable v0.7.0 evidence. After release, use the bounded pilot to prove the method against real producer bytes and actual-consumer journeys before reviewing `okf-uk-living`. |
