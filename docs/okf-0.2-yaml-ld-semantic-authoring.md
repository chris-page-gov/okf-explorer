...
| `okf-explorer` locked Python toolchain | Local implementation and validation are complete and under independent review in [PR #97](https://github.com/chris-page-gov/okf-explorer/pull/97); implementation commit `a9bb67ef` pins CPython 3.12.11 and `uv` 0.12.2, commits a cross-platform dependency lock, replaces repeated root-workflow `pip` installation with one reusable setup action, and routes governed local, CI and Foundry browser-server commands through `uv run --locked`. The exact interpreter/import check, 363 Python tests, 342 Vitest tests, 78 Node contract tests with one expected skip, type check, SBOM, deterministic double-build, componentised Site assembly and full unchanged Heritage candidate-family checks passed. The old requirements file and historical `python3` fixture declarations remain only at the exact-byte, digest-bound external publication-unit boundary; current root workflows do not execute those strings. | Merge PR #97 only after exact-head CI passes. Do not fold this infrastructure change into the authoring-method draft or mutate the frozen Heritage candidate evidence. |
