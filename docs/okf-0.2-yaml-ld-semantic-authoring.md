...
| Explore OKF authoring method | Draft implementation under review. The [methodology review](okf-authoring-methodology-review-2026-08-12.md) applies the Land Registry retrospective without reopening that pack. Authoring Profile v1, its template, prompts and beginner guidance now add graph-reachable label coverage, a compact label index, eligible-entity semantic-link denominators and an explicitly non-release Explore OKF feedback stage. The [open-data shortlist](../research/explore-okf-open-data-test-candidates.md) recommends a bounded Coventry everyday-services pilot. | Validate and review the profile/docs change; implement opaque-label/index handling in [issue #94](https://github.com/chris-page-gov/okf-explorer/issues/94) and the planned banner/route-preserving feedback in [issue #95](https://github.com/chris-page-gov/okf-explorer/issues/95); then use the pilot as the precondition for reviewing `okf-uk-living`. |
