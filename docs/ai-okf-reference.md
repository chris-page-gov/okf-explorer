...
Advanced retrieval reference. These URL-based instructions require a host that
can retrieve the relevant files. Start with the [AI access chooser](ai-okf-usage.md)
to establish what your current AI can actually read.
...
Do not claim that every corpus record is RDF-materialised when only the
...
- loads or summarises the whole corpus when a search shard would answer the
  question;
