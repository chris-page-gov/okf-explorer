...
| Exact-deployment verification | Correction implemented; deployment gate required | Run [32175666804](https://github.com/chris-page-gov/okf-explorer/actions/runs/32175666804) proved exact commit `f981130c`, identity and route bytes, then correctly failed on the generated registry's missing project-relative favicon. The renderer and diagnostics are fixed and focused tests pass; the final main candidate must pass the same clean-console Chrome gate. |
