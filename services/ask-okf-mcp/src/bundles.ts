...
import evidenceConnectManifestText from '../vendor/okf-dwp-evidence-connect-corpus-manifest.json?raw';
...
import { BUNDLE_VERSION, PRIOR_BUNDLE_VERSION, HOUSEHOLD_BUNDLE_VERSION, LEGACY_BUNDLE_VERSION, STAFF_BUNDLE_VERSION, PREVIOUS_BUNDLE_VERSION, verifyBundledContext, verifyBundledCorpus,
...
let evidenceConnect: ReturnType<typeof verifyBundledCorpus> | undefined;
...
export function loadApprovedSource(version = BUNDLE_VERSION): Promise<ApprovedSource> {
  ...
  if (version === BUNDLE_VERSION) return evidenceConnect ??= verifyBundledCorpus(evidenceConnectManifestText);
  if (version === PRIOR_BUNDLE_VERSION) return corpus ??= verifyBundledCorpus(manifestText, version);
