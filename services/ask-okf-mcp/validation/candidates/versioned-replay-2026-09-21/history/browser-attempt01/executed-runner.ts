...
assert.ok((await lstat(dirname(output))).isDirectory()); await mkdir(output);
...
for (const [path, digest] of Object.entries(build.inputs)) assert.equal(sha(await readFile(resolve(serviceRoot, path))), digest);
...
try {
  ...
  const direct = await resolveReplay(source, recipe, async () => { throw new Error('No fetch'); });
  ...
  while (await page.locator('#more-records').isVisible()) {
    ...
    await page.waitForFunction(count => document.querySelectorAll('#records article').length > count, before);
    assert.ok(++pages < 10);
  }
  ...
  for (const path of ['historical-replay.png', 'historical-unavailable.png', 'executed-runner.ts']) {
