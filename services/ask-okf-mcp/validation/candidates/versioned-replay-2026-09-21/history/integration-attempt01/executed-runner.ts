...
for (const path of [dwpRoot, dirname(output)]) assert.ok((await lstat(path)).isDirectory() && !(await lstat(path)).isSymbolicLink());
await mkdir(output); // Exclusive: never overwrite any earlier pass or failure.
const started = new Date().toISOString();
const buildRaw = await readFile(resolve(serviceRoot, 'dist/build-receipt.json'));
const build = JSON.parse(buildRaw.toString());
const originalRaw = await readFile(resolve(serviceRoot, 'validation/approved-versions-0.5.0.json'));
...
for (const [path, digest] of Object.entries(build.inputs)) assert.equal(hash(await readFile(resolve(serviceRoot, path))), digest);
for (const [path, digest] of Object.entries(build.outputs)) assert.equal(hash(await readFile(resolve(serviceRoot, path))), digest);
...
try {
  ...
  for (let caseIndex = 0; caseIndex < selectedCases.length; caseIndex++) {
    ...
    if ('manifest' in source) {
      const raw = new TextEncoder().encode(JSON.stringify(source.manifest, null, 2) + '\n');
