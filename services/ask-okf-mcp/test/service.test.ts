...
test('host-compatible /okf/mcp alias returns the same complete package as /mcp', async () => {
  ...
  try {
    ...
    assert.ok(CORPUS_EXPLORER_URL.endsWith('%2Fstructured-context%2Fevidence-connect-explorer.json'));
    ...
    assert.match(html, /Staff-task profiles declare missing scope, legal and review obligations/);
    assert.match(html, /https:\/\/chris-page-gov.github.io\/okf-dwp\/docs\/learning-path.html/);
    assert.match(html, /Discovery cards and source references help navigation/);
    assert.match(html, /evidence-workbench\/tools-manifest.json/);
