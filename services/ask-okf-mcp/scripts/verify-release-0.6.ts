...
import { HOUSEHOLD_BUNDLE_VERSION, BUNDLE_VERSION, STAFF_BUNDLE_VERSION, PREVIOUS_BUNDLE_VERSION, LEGACY_BUNDLE_VERSION, APPROVED_VERSIONS } from '../src/registry.ts';
import { canonicalJson } from '../../../apps/okf-explorer/src/lib/context/index.ts';
import { boundedFile, noLinks, safeRepositoryPath, PUBLIC_QUESTION, PUBLIC_UNKNOWN_QUESTION } from './verify-versioned-remote.ts';
const noSymlinks = noLinks;
const freshDirectory = async (path: string) => { await noLinks(dirname(path)); await mkdir(path); };
async function verifiedBuild(root: string) {
  const raw = await boundedFile(resolve(root, 'dist/build-receipt.json'), 65536); const value = JSON.parse(raw.toString());
  assert.equal(value.schema, 'okf-remote-mcp-build.v1'); assert.equal(value.service_version, '0.6.0');
  assert.ok(Object.keys(value.inputs).length <= 128); assert.deepEqual(Object.keys(value.outputs).sort(), ['dist/node.mjs', 'dist/server/index.js']);
  for (const [path, expected] of Object.entries({ ...value.inputs, ...value.outputs })) {
    assert.ok(/^(src\/[^/]+\.ts|scripts\/build\.mjs|vendor\/okf-dwp-[a-z-]+\.json|vendor\/engines\/[a-f0-9]{40}\/(index\.ts|corpus\.ts|types\.ts|manifest\.json)|package(?:-lock)?\.json|dist\/(node\.mjs|server\/index\.js)|\.\.\/\.\.\/apps\/okf-explorer\/src\/lib\/context\/(index|corpus|delivery)\.ts|\.\.\/\.\.\/profiles\/context-assembly\/v1\/(common|package)\.schema\.json)$/.test(path), 'Unapproved build input');
    assert.equal(hash(await boundedFile(resolve(root, path))), expected);
  }
  return { raw, value };
}
...
const gitBytes = (version: string, path: string, expectedBytes: number, expectedHash: string) => {
  ...
  if (!files[spec]) { assert.ok(Object.keys(files).length < 512 && sourceBytes + expectedBytes <= 64 * 1024 * 1024, 'Local source census exceeds its bound'); sourceBytes += expectedBytes; }
  const size = execFileSync('git', ['cat-file', '-s', spec], { cwd: dwpRoot, maxBuffer: 100, timeout: 10000 }).toString().trim();
  assert.equal(size, String(expectedBytes));
  const raw = execFileSync('git', ['show', spec], { cwd: dwpRoot, maxBuffer: expectedBytes + 1, timeout: 10000 });
...
try {
  ...
  await retain('remote-verifier.ts', await boundedFile(resolve(serviceRoot, 'scripts/verify-versioned-remote.ts')));
  ...
  const selectedCases = [
    { id: 'partner-care-home-512k', version: BUNDLE_VERSION, question: PUBLIC_QUESTION, budget: { max_bytes: 524288 } },
    { id: 'partner-unknown-512k', version: BUNDLE_VERSION, question: PUBLIC_UNKNOWN_QUESTION, budget: { max_bytes: 524288 } },
    ...verificationCases(HOUSEHOLD_BUNDLE_VERSION, PREVIOUS_BUNDLE_VERSION, LEGACY_BUNDLE_VERSION, STAFF_BUNDLE_VERSION).compact
  ];
  assert.deepEqual([...new Set(selectedCases.map((row: any) => row.version))], [...APPROVED_VERSIONS]);
  for (let caseIndex = 0; caseIndex < selectedCases.length; caseIndex++) {
    ...
    if ('manifest' in source) {
      ...
      const vendor = testCase.version === BUNDLE_VERSION ? 'okf-dwp-corpus-manifest.json' : testCase.version === HOUSEHOLD_BUNDLE_VERSION ? 'okf-dwp-household-corpus-manifest.json' : testCase.version === STAFF_BUNDLE_VERSION ? 'okf-dwp-staff-corpus-manifest.json' : 'okf-dwp-previous-corpus-manifest.json';
    ...
    let oldContext: any;
    for (const engine of ENGINES.filter(engine => engine.source_versions.includes(testCase.version))) {
      ...
      if (engine.engine_id === PREVIOUS_ENGINE_ID) {
        oldContext = direct;
        assert.equal(hash(canonical), original.compact_delivery_versions.find((row: any) => row.bundle_version === testCase.version).package_canonical_sha256,
      ...
      try {
        ...
        cases.push({ id: testCase.id, question_sha256: hash(testCase.question), context_budget: budget, source_version: testCase.version, engine_id: engine.engine_id,
  ...
  const household = cases.filter(row => row.source_version === HOUSEHOLD_BUNDLE_VERSION);
  assert.notEqual(household[0].context_id, household[1].context_id);
  assert.equal(cases.length, 10); assert.equal(cases[0].evidence_status, 'insufficient');
  assert.equal(cases[1].records, 0); assert.equal(cases[1].evidence_status, 'insufficient');
  const receipt = { schema: 'okf-release-replay-observation.v1', classification: 'undeployed-local-release-0.6.0',
    ...
    worker_sha256: build.outputs['dist/server/index.js'], source_bytes: sourceBytes, source_file_limit: 512, source_byte_limit: 64 * 1024 * 1024, files, cases, retained,
  ...
  console.log(JSON.stringify({ passed: cases.length, historical_replays: 4, files: Object.keys(files).length, worker: receipt.worker_sha256 }));
} catch (error) {
