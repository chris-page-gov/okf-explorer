  ...
  const fetcher: typeof fetch = async (input, init) => {
    ...
    if (body) {
      ...
      if (method === 'tools/call') {
        ...
        assert.equal(message.params.arguments?.question, PUBLIC_QUESTION, 'Only the explicit public acceptance question may be sent');
      }
    }
...
async function preparation(args: Arguments) {
  ...
  try {
    ...
    for (const [index, pair] of pairs.entries()) {
      ...
      cases.push({ id: `pair-${String(index).padStart(2, '0')}`, request, ...result,
    ...
    cases.push({ id: 'original-0.5-care-home-unspecified-engine', request: historicalRequest, ...historical, inspectRecord: true });
    const enriched = [];
...
export async function prepareDelivery(item: any, origin: string, helpers: { delivery: any; bindEvidenceRead: any; reviewLink: any }) {
  const { delivery, bindEvidenceRead, reviewLink } = helpers;
      ...
      const pinned = { bundle: 'okf-dwp' as const, version: item.request.version, question: PUBLIC_QUESTION, budget, context_id: context.context_id, engine_id: identity.engine_id };
...
export async function verifyDelivery(client: { callTool(args: any): Promise<any> }, item: any, origin: string) {
  ...
  return { complete, receipt: { id: item.id, source_version: item.request.version, engine_id: item.identity.engine_id,
...
export async function main(argv = process.argv.slice(2)) {
  ...
  try {
    ...
    const plan = { schema: 'okf-versioned-remote-plan.v1', classification: 'offline-local-reference-plan',
      ...
      census: { cases: prepared.cases.map(item => ({ id: item.id, calls: item.calls, package_bytes: Buffer.byteLength(canonical(item.context)),
      ...
      question_sha256: sha(PUBLIC_QUESTION), full_ask_okf_calls: 0, network_calls: 0,
    ...
    await writeFile(resolve(args.output, 'observation.json'), JSON.stringify({ schema: 'okf-versioned-remote-verification.v1',
      ...
      limitations: ['Hosting evidence is required to attest deployed Worker bytes; health alone establishes only reported source/engine/service identities.',
        ...
        'The retained packages contain only the fixed public acceptance question. Anonymous user questions are never sampled.'] }, null, 2) + '\n', { flag: 'wx' });
