...
else { const info = await lstat(output); assert(info.isDirectory() && !info.isSymbolicLink(), 'Unsafe receipt directory'); }
...
const fromGit = (checkout, commit, name) => execFileSync('git', ['show', `${commit}:${safe(name)}`], { cwd: checkout, maxBuffer: 32 * 1024 * 1024 });
async function bounded(name, limit = 8 * 1024 * 1024) {
  const full = path.join(output, safe(name)); const info = await lstat(full);
  assert(info.isFile() && !info.isSymbolicLink() && info.size <= limit, 'Invalid retained file');
  assert((await realpath(full)).startsWith((await realpath(output)) + path.sep), 'Receipt escaped directory');
...
for (const name of files) {
  const original = fromGit(root, BASELINE, ENGINE + name);
  assert.deepEqual(await readFile(path.join(here, 'baseline', name)), original, 'Baseline archive differs from commit');
  const candidate = checking ? await bounded('candidate/' + name) : await readFile(path.join(root, ENGINE, name));
