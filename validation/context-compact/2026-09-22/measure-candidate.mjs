/** Candidate compact-delivery observation. Local hash-bound reads, no models/network. */
...
const receipt = { schema: 'okf-logical-compact-observation.v1', observed_at: new Date().toISOString(),
  dwp_checkout_base: execFileSync('git', ['-C', dwp, 'rev-parse', 'HEAD'], { encoding: 'utf8' }).trim(),
  source_admission: 'Hash-bound local candidate. The checkout base is not a claim that the candidate files occur in that commit; each actual read is bound below.',
  comparison: 'Current-candidate transport check. The retained inline baseline may use the same admitted engine; this is not an earlier-source semantic comparison.',
