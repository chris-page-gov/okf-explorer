...
let browser:any, server:any, page:any;const requests:any[]=[], errors:string[]=[], assertions:string[]=[];
...
try {
  ...
  report.first_selection=await page.evaluate(()=>({url:location.href,href:document.querySelector('#examples a')?.getAttribute('href'),active:document.activeElement?.outerHTML}));
  await page.waitForFunction(()=>document.querySelector('#status')?.textContent?.includes('unavailable')||document.querySelector('#example button'));
  assert.ok(!(await page.locator('#status').textContent()).includes('unavailable'),await page.locator('#status').textContent());
  ...
  for(const entry of actual.cases) {
    ...
    await page.getByRole('heading',{name:entry.title,exact:true,level:2}).waitFor();
  ...
  await page.getByRole('heading',{name:actual.cases[0].title,exact:true,level:2}).waitFor();
...
}catch(error:any){report.outcome='failed';report.failure={name:error.name,message:error.message};if(page){report.browser_failure_state={url:page.url(),status:await page.locator('#status').textContent().catch(()=>null)};await page.screenshot({path:resolve(output,'failure.png')}).catch(()=>{});}process.exitCode=1;}
finally{
