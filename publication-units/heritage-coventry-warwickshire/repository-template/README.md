...
the same commit is deployed by Pages. Dispatch terminal assurance at that tag
with a 40-hex OKF Explorer assurance commit. The terminal gate reconstructs
...
commit and dispatch promotion at that R2 tag with the successful terminal run
ID and the same exact 40-hex OKF Explorer assurance commit. Both workflows
finish by checking GitHub's verified release attestation
