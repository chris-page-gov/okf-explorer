...
PRESETS: dict[str, Preset] = {
    ...
    "okf-els-api": Preset(
        ...
        "bundle/index.md",
        "descriptor-yaml-ld",
        ("source/", "scripts/build_bundle.py", "scripts/okf_v02.py"),
        (
            ("bundle/okf-bundle.yamlld", "semantic-yaml-ld", True),
            ("bundle/okf-bundle.jsonld", "semantic-json-ld", True),
            ("bundle/context/okf-els-api.jsonld", "semantic-context", True),
            ("bundle/okf-explorer.json", "explorer-runtime", True),
            ("bundle/data/manifest.json", "relationship-runtime-manifest", True),
            ("bundle/data/relationships-0.json", "relationship-runtime", True),
            ("bundle/data/standards/term-validation.json", "semantic-validation", True),
        ),
        "runtime-assertion-migration",
        "migration-pending",
        "json-large-corpus-chunks",
        ...
        (
            "The generated YAML-LD and JSON-LD documents are bounded catalogue and service descriptors; the separate Explorer relationship rows are not reified okf:RelationshipAssertion nodes.",
            "The current relationship rows lack absolute source and target IRIs, inverse labels, assertion scope, authority, derivation and rights, so rich assertion migration remains pending.",
            "The frozen metadata review does not execute or live-verify the internal/private upstream API and includes no observation values.",
        ),
    ),
    ...
    "okf-planning": Preset(
        ...
        (
            ("bundle/okf-bundle.yamlld", "semantic-yaml-ld", True),
            ("bundle/okf-bundle.jsonld", "semantic-json-ld", True),
            ("bundle/context/okf-planning.jsonld", "semantic-context", True),
            ("bundle/okf-explorer.json", "explorer-runtime", True),
            ("bundle/data/manifest.json", "relationship-runtime-manifest", True),
            ("bundle/data/relationships-*.json", "relationship-runtime", True),
            ("bundle/data/standards/evaluation.json", "semantic-validation", True),
        ),
        "runtime-assertion-migration",
        "migration-pending",
        "json-large-corpus-chunks",
        ("python3 scripts/build_bundle.py",),
        ("python3 scripts/check_okf.py", "pytest", "git diff --exit-code"),
        (
            "The generated YAML-LD and JSON-LD documents are additive catalogue mappings; they do not certify Planning Data England or conformance with DCAT, PROV or SKOS.",
            "The current rule-derived Explorer relationships are sparse discovery links without stable assertion IDs, absolute predicates and endpoint IRIs, inverse labels, governed status and scope, authority, derivation, observation time, evidence or rights; rich assertion migration remains pending.",
            "No entity reconciliation or human review is claimed for the rule-derived catalogue relationships.",
        ),
